...
dir.close()
user_name = list(data['user'].unique())
...
pickle.dump(user_name, dir_user)
dir_user.close()
...
class Find(object):
    ...
    def find_near_neigh(self):
        ...
        similars_user = similars_user[user_num][1:]
...
class Calculate_rating(Find):
    ...
    def calcul_rating(self):
        ...
        for colnum in range(narray.shape[1]):
            sum_ = 0
            rating = 0
            for i in range(1, len(narray[:, colnum])):
                sum_ += float(narray[:, colnum][i]) + float(similar_diatance[i])
            rating = sum_ / similar_diatance.sum()
